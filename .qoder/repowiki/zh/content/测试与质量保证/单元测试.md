...
## 更新摘要
**变更内容**   
- **重大更新**：扩展引擎测试覆盖，新增191行测试代码，总计219个核心测试用例，确保流式功能的可靠性
...
**重大更新** 本次更新显著增强了引擎测试基础设施，新增了191行测试代码，总计219个核心测试用例，特别强化了流式功能的可靠性保障。同时新增了字幕烧录功能测试、分割组件测试、章节处理能力测试和命令行接口测试，全面提升代码质量和可靠性保障。
...
- **转录引擎（Transcribe 与 Engines）**
  - **重大更新**：使用 sys.modules 注入 faster_whisper 或 funasr 的 mock，验证 transcribe 输出结构与进度回调
  - **新增**：引擎注册表 create_engine、健康检查异常路径、ffprobe 辅助函数错误分支
  - **强化**：流式功能测试覆盖，确保219个核心测试用例的可靠性
...
- [test_engines.py:1-219](file://video_splitter/tests/test_engines.py#L1-L219)
...
- [test_chapter.py:85-107](file://video_splitter/tests/test_chapter.py#L85-107)
- [test_chapter.py:109-213](file://video_splitter/tests/test_chapter.py#L109-213)
- [test_chapter.py:215-310](file://video_splitter/tests/test_chapter.py#L215-310)
...
- 目标
  - **重大更新**：验证 SRT 生成、token 估算、transcribe 输出结构、进度回调；引擎注册表与健康检查异常路径
  - **新增**：流式功能测试覆盖，确保219个核心测试用例的可靠性
- 关键策略
...
- [test_engines.py:72-219](file://video_splitter/tests/test_engines.py#L72-L219)
...
- [test_transcribe.py:105-242](file://video_splitter/tests/test_transcribe.py#L105-L242)
- [test_engines.py:22-219](file://video_splitter/tests/test_engines.py#L22-L219)
- [test_engines.py:72-219](file://video_splitter/tests/test_engines.py#L72-L219)
...
**重大更新** 本次更新显著增强了测试基础设施，特别是引擎测试覆盖扩展到219个核心测试用例，新增了字幕烧录功能测试、分割组件测试、章节处理能力测试和命令行接口测试，进一步提升了代码质量和可靠性保障。流式功能的可靠性得到了全面验证。
...
- [test_engines.py:32-219](file://video_splitter/tests/test_engines.py#L32-L219)
- [test_subtitle_burn.py:1-29](file://tests/test_subtitle_burn.py#L1-L29)
- [test_cli.py:1-135](file://video_splitter/tests/test_cli.py#L1-L135)
